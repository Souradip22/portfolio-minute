...
export default function DomainPage({ formValues }: { formValues: any }) {
  const { shortname, fullname, bio, phone, email, skills, projects } =
    formValues;
  return (
    <div className="mx-auto flex w-[70%] flex-col h-screen justify-between gap-4 p-4 overflow-y-scroll">
      ...
      <div className="rounded-2xl bg-white p-6 shadow dark:bg-black dark:shadow-dark lg:col-span-2 lg:p-10">
        <div className="flex flex-col-reverse items-start  lg:flex-row justify-between">
          ...
          <div className="flex items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-purple-100 px-4 py-2 text-center text-base font-medium leading-none text-primary dark:bg-dark-2 lg:text-lg">
            ...
            <div className="flex gap-x-3 gap-y-3 md:gap-y-3 md:gap-x-3 flex-wrap my-2 w-full mt-4">
              {skills &&
                skills.map((item: any, index: any) => (
                  // <span key={`${index}-skill`}>nextjs </span>
                  <button
                    key={`${index}-skill`}
                    className="bg-gray-200 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-md  text-[12px]"
                  >
                    {item.label}
                  </button>
                ))}
